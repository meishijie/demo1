...
class Game:
    ...
    def _draw_bottom_bar(self) -> None:
        ...
        self._draw_text(4, SCREEN_HEIGHT - 9, self.status_text[:40], 7, compact=True)
        
        # Terrain info on the right
        hovered_tile = self._screen_to_map_tile(pyxel.mouse_x, pyxel.mouse_y)
        if hovered_tile:
            ...
            terrain_str = f"地形: {terrain.name}"
            if def_bonus > 0:
                terrain_str += f" (防+{def_bonus})"
            
            tw = len(terrain_str) * 4 # Approximate width
            self._draw_text(SCREEN_WIDTH - tw - 50, SCREEN_HEIGHT - 9, terrain_str, 11, compact=True)
    ...
    def _draw_hover_unit_info(self) -> None:
        ...
        if unit:
            ...
            # If no unit and not a special preview Case, return early to avoid terrain-only box
            if not is_attack_preview:
                return
            
        terrain_type = unit_system.TerrainType(terrain.name.lower())
        if is_attack_preview:
            lines.append("--- 战斗预测 ---")
